...
uint64_t travel(const char *const grid, const int width, const int len, const int s_pos, const int e_pos, const direct_t s_dir) {
    ...
    while (v_list_len > 0) {
        ...
        c_node->s = Closed;
        // printf("c_pos=(%d, %d) -> %c\n", c_pos % width, c_pos / width, grid[c_pos]);
        if (grid[c_pos] == '#') {
            continue;
        }
...
uint64_t travel2(const char *const grid, const int width, const int len, const int s_pos, const int e_pos, const direct_t s_dir) {
    printf("spos = %d; epos = %d\n", s_pos, e_pos);
    uint64_t res = 0;
    int visit_list[4096];
    int v_list_len;
    ...
    while (v_list_len > 0) {
        ...
        // printf("c_pos=%d -> (%d, %d, %d) -> %c\n", c_index, g_index % width, g_index / width, c_index / len, grid[g_index]);
        c_node->s = Closed;
        if (grid[g_index] == '#') {
            continue;
        }
        ...
        for (int i = 0; i <= 1; i++) {
            ...
            node_t *n_node = nodes + n_index;
            // printf("LR %d %d -> cdir: %d ndir: %d dist: %lu\n", n_index, n_node->s, c_dir, n_dir, n_node->dist);
            if (n_node->s == Closed) {
                continue;
            }
    ...
    printf("\n\n------\n\n");
    {
        ...
        while (pos_len > 0) {
            ...
            int neigh[] = { n_pos + len * c_dir, ((p - len) + node_len) % node_len, (p + len) % node_len };
            ...
            for (int j = 0; j < len_neigh; j++) {
                ...
                if (n_node->dist < nodes[p].dist && grid[g_index] != '#' && n_node->dist + offset[j] == nodes[p].dist) {
                    pos_list[pos_len++] = c_index;
                }
            }
            for (int i = 0; i < pos_len; i++) {
                printf("c_pos=%d -> (%d, %d, %d) |||| ", pos_list[i], (pos_list[i] % len) % width, (pos_list[i] % len) / width, pos_list[i] / len);
            }
            printf("\n");
        }
        for (int i = 0; i < len; i++) {
